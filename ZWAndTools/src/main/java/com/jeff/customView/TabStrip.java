...
public class TabStrip extends HorizontalScrollView {
     ...
     * 指示器宽度是否有屏幕宽度算出（默认是）
     */
    private boolean indicatorWidthDivideByScreen=true;
    /**
     * ViewPager
     */
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr,
        ...
        for (int i = 0; i < n; i++) {
            ...
            } else if (attr == R.styleable.TabStrip_indicatorWidthDivideByScreen) {
                indicatorWidthDivideByScreen = typedArray.getBoolean(attr, true);
...
//                LayoutParams.MATCH_PARENT, 1.0f);
    ...
    private void addTab(final int position, CharSequence title) {
        ...
        int totleWidth=getScreenWidth();
        if(!indicatorWidthDivideByScreen){
            totleWidth=container.getMeasuredWidth();
        }
        expandedTabLayoutParams = new LinearLayout.LayoutParams(totleWidth/ tabCount,
                LayoutParams.MATCH_PARENT, 1.0f);
    ...
    public void setUpdataDataListener(UpdateListener listener) {
    ...
    private void updateTabStyle() {
        ...
        if (null != mListener) {
            mListener.updateData(selectedPosition);
        }
    }
